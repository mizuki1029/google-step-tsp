...
public class tsp_ga_64 {
	...
	public static void main(String[] args) {
        ...
        System.out.println("100000th Generations's best route");
        howGenerations(100000);
        ...
        writeFile("output_3.csv", bestRoute);
	}
	...
	public static double sumDistance(int route[]) {
		...
		for (int i = 0; i < route.length-1; i++) {
			sumDistance += distances[route[i]][route[i+1]];
		}
		sumDistance += distances[route[0]][route[route.length-1]];
		return sumDistance;
	}
	...
	public static void parentRoutes(){
		for(int i = 0; i < parentRoutes.length; i++){
			System.arraycopy(routes[i], 0, parentRoutes[i], 0, parentRoutes[i].length);
		}
	}
